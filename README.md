...
cd code
```
Within this directory, we will create another folder to contain our lab assignments code. You can also create other directories for other coding projects you may undertake.

For now, let's download the cs111_assignments repository.
More specifically, we will "clone" the cs111_assignments github repository into the code directory.
Type
```bash
...
ls
```
As a tip, you can press the TAB key to autocomplete certain commands.  This is particularly useful when typing filepaths, as it also provides you with validation that the filepath exists if autocomplete works, or conversely the filepath does not exist if autocomplete fails. For example, try typing '**cd as**' and then, without pressing Enter, hit the TAB key. The command should autocomplete to **cd assignment1**.

## Setup Eclipse for Assignment 1

Start Eclipse and set your workspace to the cs111_assignments directory.

Now we will have Eclipse create a new project based on the contents of the assignment1 folder inside the cs111_assignments directory. In order for this to work properly, the new project must be named "assignment1" (without quotes). 

To do this, in Eclipse, click File -> New -> Java Project
Enter **assignment1** as that Project name.
Click Finish
